...
(
    ...
    Dia_de_la_semana VARCHAR(20) CHECK (Dia_de_la_semana IN ('LUNES', 'MARTES', 'MIERCOLES', 'JUEVES', 'VIERNES', 'SABADO', 'DOMINGO')) NOT NULL,
...
(
    Id_equipamiento     SERIAL PRIMARY KEY NOT NULL,
    Id_aula             INTEGER            NOT NULL,
    Nombre              VARCHAR(100)       NOT NULL,
    Fecha_mantenimiento DATE               NOT NULL,
    Descripcion         VARCHAR(200),
    UNIQUE (Id_aula, Nombre),
...
CREATE OR REPLACE FUNCTION calcular_duracion_interval(hora_inicio TIME, hora_fin TIME)
    RETURNS INTERVAL AS
$$
BEGIN
...
$$
BEGIN
    NEW.Duracion := calcular_duracion_interval(NEW.hora_inicio, NEW.hora_fin);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
